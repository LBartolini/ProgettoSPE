...
Configuration is loaded from config.json (sources, distances, arrival rates, sink service rate).

sink (single server, state-dependent probabilistic routing):
...
LAMBDA_sink = config["sink"]["service_rate"]

# ── Reproducibility ──────────────────────────────────────────────────────────
#rng = np.random.default_rng(SEED)
rng = np.random.default_rng()

# ── sink routing probability table ──────────────────────────────────────────
# Maps sink occupancy at the moment of a packet's arrival to the probability
...
ROUTING_PROB_TABLE: dict[int, float] = {
    0: 0.999,   # server idle  -> always admit
...
stats = {
    ...
    "success_times":    [],
    "discard_times":    [],
    "occ_area":         0.0,  # time-weighted occupancy integral (after warm-up)
    "occ_last_time":    0.0,  # last time the occupancy changed
}


def _update_occ_area(now: float, occupancy: int) -> None:
    """Accumulate the time-weighted occupancy integral over [WARM_UP, SIM_TIME]."""
    last = stats["occ_last_time"]
    if now > WARM_UP:
        effective_start = max(last, WARM_UP)
        stats["occ_area"] += occupancy * (now - effective_start)
    stats["occ_last_time"] = now


def source(env, name, interarrival_lambda, sink_resource, sink_occupancy):
    """
    ...
    while True:
        ...
        # Fire-and-forget
        env.process(sink_arrive(env, sink_resource, sink_occupancy))


def sink_arrive(env, sink_resource, sink_occupancy):
    """
    State-dependent probabilistic routing at the sink:
      - Read current occupancy from sink_occupancy[0].
    ...
    if arrival_time >= WARM_UP:
        stats["sink_arrivals"] += 1
        stats["arrival_times"].append(arrival_time)

    # Both paths go through the server (occupancy updated for all)
    _update_occ_area(env.now, sink_occupancy[0])
    sink_occupancy[0] += 1
    with sink_resource.request() as req:
        yield req
        service_time = rng.exponential(1.0 / LAMBDA_sink)
...
def run_simulation():
    ...
    sink_resource  = simpy.Resource(env, capacity=1)   # single server
    sink_occupancy = [0]   # mutable counter: packets currently at the sink

    # Instantiate sources from config
    for s in config["sources"]:
        env.process(source(env, s["name"], s["arrival_rate"],
                           sink_resource, sink_occupancy))

    env.run(until=SIM_TIME)
    # Final update to account for the last occupancy interval
    _update_occ_area(SIM_TIME, sink_occupancy[0])
    return env
...
if __name__ == "__main__":
    ...
    total_arrivals = stats["sink_arrivals"]
    ...
    # Effective arrival rate at sink:
    #   Propagation runs independently of the IAT loop, so packets arrive at
    #   the sink at exactly the source emission rate lambda_i.
    lambda_total = sum(s["arrival_rate"] for s in config["sources"])
    # M/M/1/1 theoretical loss probability: rho/(1+rho), rho = lambda/mu
    rho            = lambda_total / LAMBDA_sink
    theoretical_loss = rho / (1 + rho)
    # M/M/1/1 mean occupancy: E[N] = 1*pi_1 = rho/(1+rho)
    theoretical_avg_occ = rho / (1 + rho)
    avg_occupancy  = stats["occ_area"] / effective_time

    print(f"\n  {'Metric':<40} {'Simulated':>10}  {'Theoretical':>12}")
    print(f"  {'-'*40}   {'-'*10}  {'-'*12}")
    print(f"  {'Sink arrival rate (pkts/s)':<40} {arrival_rate:>10.4f}  {lambda_total:>12.4f}")
    ...
    print(f"  {'Avg Sink occupancy (pkts)':<40} {avg_occupancy:>10.4f}  {theoretical_avg_occ:>12.4f}")
    print(f"\n  Total packets arrived at Sink : {total_arrivals}")
